...
const localeToCacheKey = (locale: string) => `https://mock-domain/localization/${locale}.txlns.ts`;
...
const TestComponent: React.FC<{fileLoaders: Partial<LocalizationFileLoaderMap<Partial<TestLocalizedStrings>>>}> = ({fileLoaders}) => {
...
describeUnitTest("useTranslations hook", () => {
    ...
    it("should load only the files it needs, and each file only once", async () => {
        ...
        expect(EN_LOADER_MOCK).toBeCalledTimes(1); // expect "en" (default) loader not called a second time
        expect(FR_LOADER_MOCK).toBeCalledTimes(1); // expect new needed loader called
        expect(BS_LOADER_MOCK).not.toBeCalled();   // didn't load unneeded localized strings
        ...
        expect(EN_LOADER_MOCK).toBeCalledTimes(1); // expect "en" (default) loader not called a second time
        expect(FR_LOADER_MOCK).toBeCalledTimes(1); // expect "fr" loader not called a second time
        expect(BS_LOADER_MOCK).toBeCalledTimes(1); // expect new needed loader called 
